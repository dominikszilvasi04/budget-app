...
function GoalsPage() {
  ...
  const handleAddGoalSubmit = async (event) => {
    ...
    if (isNaN(parsedTarget)) {
      setAddGoalError('Valid positive target amount is required.');
      return;
    }
  ...
  const handleAddContributionSubmit = async (event) => {
    ...
    try {
      ...
      setGoals((prevGoals) =>
        prevGoals.map((g) => (g.id === goalId ? response.data.updatedGoal : g))
      );
  ...
  const handleUpdateGoalSubmit = async (event) => {
    ...
    try {
      ...
      setGoals((prevGoals) =>
        prevGoals
          .map((g) => (g.id === goalId ? response.data.updatedGoal : g))
  ...
  const handleDeleteGoal = async (goalId, goalName) => {
    if (
      !window.confirm(
        `Are you sure you want to delete the goal "${goalName}"? All contributions will also be deleted.`
      )
    ) {
      return;
    }
  ...
  if (loading) {
    return <div className="page-status">Loading goals...</div>;
  }
  if (error) {
  ...
  return (
    <div className="goals-page-container">
      ...
      <p className="section-subtitle">
        Track progress clearly and contribute to each goal from one place.
      </p>
            ...
            {goals.map((goal) => {
              ...
              return (
                <div key={goal.id} className="goal-item-card">
                  ...
                  {goal.target_date && (
                    <p className="goal-target-date">Target Date: {goal.target_date}</p>
                  )}
                  {goal.notes && <p className="goal-notes">Notes: {goal.notes}</p>}
                  <div className="goal-card-actions">
                    <button
                      onClick={() => handleOpenContributionPopup(goal)}
      ...
      {contributionPopupGoal && (
        <div className="popup-overlay contribution-overlay" onClick={handleCloseContributionPopup}>
          <div
            className="popup-content contribution-popup-content"
          ...
          >
            ...
            <form onSubmit={handleAddContributionSubmit} className="contribution-form">
              ...
              <div className="options-button-group">
                <button
                  type="button"
                  onClick={handleCloseContributionPopup}
                  disabled={isAddingContribution}
                >
                  Cancel
                </button>
            ...
            {isGoalRenameMode ? (
              <form onSubmit={handleUpdateGoalSubmit} className="rename-form goal-rename-form">
                ...
                <div className="options-button-group">
                  <button
                    type="button"
                    onClick={handleCancelGoalRename}
                    disabled={isProcessingGoalAction}
                  >
                    Cancel
                  </button>
              ...
              <div className="options-button-group">
                <button
                  ...
                  onClick={() => handleDeleteGoal(optionsPopupGoal.id, optionsPopupGoal.name)}
                  disabled={isProcessingGoalAction}
                >
                  {' '}
                  Delete{' '}
                </button>
                <button onClick={handleTriggerGoalRename} disabled={isProcessingGoalAction}>
                  Rename / Edit
                </button>
                <button onClick={handleCloseGoalOptionsPopup} disabled={isProcessingGoalAction}>
                  Cancel
                </button>
              </div>
            )}
