...
const chartOptions = {
  ...
  plugins: {
    ...
    title: {
      display: true,
      text: 'Budget Allocation by Category (Current Month)',
      font: { size: 16 },
    },
    tooltip: {
      callbacks: {
...
function BudgetPage() {
  ...
  const rolloverFromPreviousMonth = async () => {
    ...
    try {
      ...
      setRolloverStatus(
        `${response.data.copiedRowCount} budget row(s) copied from previous month.`
      );
  ...
  const pieChartData = useMemo(() => {
    ...
    const filteredBudgets = budgets.filter(
      (budget) => budget.budget_amount > 0 && categoryTypeByIdentifier[budget.id] === 'expense'
    );
      ...
      datasets: [
        {
          ...
          backgroundColor: filteredBudgets.map(
            (budget) => categoryColourByIdentifier[budget.id] || '#CCCCCC'
          ),
          borderColor: 'black',
          borderWidth: 1,
        },
  ...
  const updateBudgetInput = (categoryIdentifier, inputValue) => {
    setBudgets((currentBudgets) =>
      currentBudgets.map((budget) =>
        budget.id === categoryIdentifier ? { ...budget, budget_amount_input: inputValue } : budget
      )
    );
  ...
  const saveBudget = async (categoryIdentifier) => {
    ...
    const valueToSave =
      selectedBudget.budget_amount_input !== undefined
  ...
  return (
    <div className="budget-page-container">
      ...
      <p className="section-subtitle">
        Enter a monthly budget for each expense category. Changes are saved automatically.
      </p>

      <div className="history-action-row">
        <button type="button" onClick={() => shiftSelectedMonth(-1)}>
          Previous
        </button>
        <input
          type="month"
        ...
        <button type="button" onClick={rolloverFromPreviousMonth}>
          Rollover Previous Month
        </button>
      </div>
              ...
              expenseBudgets.map((budget) => {
                ...
                return (
                  <div key={budget.id} className="budget-item">
                    <label htmlFor={`budget-${budget.id}`} className="budget-item-label">
                      {budget.name}
                    </label>
                    <div className="budget-item-input-group">
                      ...
                      />
                      <span
                        className={`budget-item-status status-${saveStatusByCategoryIdentifier[budget.id]?.status}`}
                      >
                        {saveStatusByCategoryIdentifier[budget.id]?.status === 'saving' &&
                          'Saving...'}
                        {saveStatusByCategoryIdentifier[budget.id]?.status === 'saved' && 'Saved'}
                        {saveStatusByCategoryIdentifier[budget.id]?.status === 'error' &&
                          `Error: ${saveStatusByCategoryIdentifier[budget.id]?.message || 'Failed'}`}
                      </span>
                    </div>
                  </div>
                );
