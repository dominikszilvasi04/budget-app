...
const addGoal = async (req, res) => {
  ...
  if (name.length > 150) {
    return res.status(400).json({ message: 'Goal name max 150 chars.' });
  }
  ...
  try {
    const insertGoalQuery = `
            ...
            VALUES (?, ?, ?, ?, ?);
        `;
    const [insertResult] = await dbPool.query(insertGoalQuery, [
      trimmedName,
      finalTargetAmount,
      0.0,
      validTargetDate,
    ...
    const [newGoalRows] = await dbPool.query(
      'SELECT id, name, target_amount, current_amount, target_date, notes, created_at FROM goals WHERE id = ?',
      [insertedId]
    );
    if (newGoalRows.length === 0) {
      throw new Error('Failed to retrieve newly added goal.');
    }
    ...
    const formattedGoal = {
      ...
      target_date: newGoalRows[0].target_date
        ? new Date(newGoalRows[0].target_date).toISOString().split('T')[0]
        : null,
    };
...
const addContribution = async (req, res) => {
  ...
  try {
    ...
    const insertContributionQuery = `
            ...
            VALUES (?, ?, ?, ?);
        `;
    await connection.query(insertContributionQuery, [
      parsedGoalId,
      contributionAmount,
      contributionDate,
      notes,
    ...
    const updateGoalQuery = `
            ...
            WHERE id = ?;
        `;
    const [updateGoalResult] = await connection.query(updateGoalQuery, [
      contributionAmount,
      parsedGoalId,
    ]);
    ...
    const [updatedGoalRows] = await connection.query(
      'SELECT id, name, target_amount, current_amount, target_date, notes, created_at FROM goals WHERE id = ?',
      [parsedGoalId]
    );
    ...
    res
      .status(201)
      .json({ message: 'Contribution added successfully!', updatedGoal: formattedGoal });
  } catch (error) {
...
const updateGoal = async (req, res) => {
  ...
  if (name !== undefined) {
    const trimmedName = name.trim();
    if (!trimmedName)
      return res.status(400).json({ message: 'Goal name cannot be empty if provided.' });
  ...
  if (target_amount !== undefined) {
    const parsedTargetAmount = parseFloat(target_amount);
    if (isNaN(parsedTargetAmount) || parsedTargetAmount <= 0) {
      return res.status(400).json({ message: 'Valid positive target amount required.' });
    }
    const finalTargetAmount = parseFloat(parsedTargetAmount.toFixed(2));
    fieldsToUpdate.target_amount = finalTargetAmount;
    values.push(finalTargetAmount);
    setClause += (setClause ? ', ' : '') + 'target_amount = ?';
  }
  ...
  if (target_date !== undefined) {
    if (target_date === null || target_date === '') {
      ...
      return res
        .status(400)
        .json({ message: 'Invalid target date format. Use YYYY-MM-DD or leave empty.' });
    }
    fieldsToUpdate.target_date = validTargetDate;
    values.push(validTargetDate);
    setClause += (setClause ? ', ' : '') + 'target_date = ?';
  }
  ...
  try {
    ...
    const [updatedGoalRows] = await dbPool.query(
      'SELECT id, name, target_amount, current_amount, target_date, notes, created_at FROM goals WHERE id = ?',
      [goalId]
    );
    if (updatedGoalRows.length === 0) {
