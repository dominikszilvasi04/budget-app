...
const addCategory = async (req, res) => {
  ...
  try {
    ...
    const [newCategoryRows] = await dbPool.query('SELECT * FROM categories WHERE id = ?', [
      insertedId,
    ]);
...
const updateCategory = async (req, res) => {
  ...
  try {
    ...
    const [updatedCategoryRows] = await dbPool.query('SELECT * FROM categories WHERE id = ?', [
      categoryId,
    ]);
    ...
    res
      .status(200)
      .json({ message: 'Category updated successfully!', updatedCategory: updatedCategoryRows[0] });
  } catch (error) {
    console.error('Error updating category:', error);
    if (error.code === 'ER_DUP_ENTRY' && fieldsToUpdate.name) {
      return res.status(409).json({
        message: `Another category with the name '${fieldsToUpdate.name}' already exists.`,
      });
    }
